/**
 * Les neuf vignettes de partage des pages de discipline de Portet
 * (public/og/disciplines/<slug>.jpg, 1200 × 630), dans le style des
 * vignettes existantes du site : la photo de la discipline, un dégradé nuit,
 * un surtitre en police à chasse fixe, le nom en gras, deux pastilles, le
 * logo en haut à droite. Chaque vignette est propre à sa page.
 *
 * Outil de poste : il emprunte @resvg/resvg-js au dépôt de Labège et sharp
 * au dépôt de Portet ; les JPEG produits sont versés dans le dépôt de Portet.
 * Usage : node og_portet_disciplines.mjs
 */
...
const W = 1200, H = 630;
const pages = JSON.parse(readFileSync(`${PORTET}/src/disciplines.json`, "utf8")).pages;
...
mkdirSync(`${PORTET}/public/og/disciplines`, { recursive: true });
for (const p of pages) {
  ...
  <text x="100" y="${ySurtitre}" font-family="JetBrains Mono" font-size="21" letter-spacing="4" fill="#d0d8e8">DISCIPLINE · PORTET-SUR-GARONNE</text>
  ...
  const sortie = `${PORTET}/public/og/disciplines/${p.slug}.jpg`;
  writeFileSync(sortie, await sharp(png).jpeg({ quality: 84, mozjpeg: true }).toBuffer());
  console.log(`[og] ${p.slug}.jpg · ${titre.length} ligne(s) de titre`);
}
